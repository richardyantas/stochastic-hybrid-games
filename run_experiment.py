...
import argparse
import importlib
from mpc.controllers import Greedy, MPC, Uppaal
from plot_controllers import viz2
...
def _import_class(module_and_clas_name: str) -> type:
    """Import class from a module, e.g. 'mpc.controllers.UPPAAL'"""
...
def _setup_parser():
    ...
    data_class = _import_class(
        f"mpc.data.{temp_args.data_class}")
    model_class = _import_class(
        f"mpc.models.{temp_args.model_class}")
    controller_class = _import_class(
        f"mpc.controllers.{temp_args.controller_class}")
...
def main():
    ...
    data_class = _import_class(
        f"mpc.data.{args.data_class}")
    model_class = _import_class(
        f"mpc.models.{args.model_class}")
    controller_class = _import_class(
        f"mpc.controllers.{args.controller_class}")
