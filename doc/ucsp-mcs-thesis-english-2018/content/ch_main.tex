  ...
  \section{Experiments: }
    ...
    \subsection{Controllers Optimizations:}
      ...
      \subsubsection{ \emph{Reinforcement learning optimization} }
        ...
        % \textbf{\textcolor{red}{\hl{Agregar explicacion de 
        % diagrama}}}
        As shown in the diagram, the process contains two 
        steps, the first one for calculating the stochastic events 
        using probability density functions along the day,
        such as (1u) defines the probability of get an uncontrollable 
        change that alter the mode if $u \rightarrow u'$ then the valve 
        paremeter wil increase in 1 such as (2u).

        The second one is about the controllable computation, after to 
        select a mode corresponding to the current zonotope, 
      ...
      importance to the temperatures distance and the energy consumption saving in the optimization, 
      the mean temperature error in the greedy method \autoref{fig:resulta} is : $2.07$ 
      and the reinforcement learning optimization is:  $3.42$ and also the saved energy 
      consumption using uppaal with respect to 
